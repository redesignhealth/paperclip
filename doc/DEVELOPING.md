...
- Managed git-worktree runtime startup also runs `scripts/provision-worktree-runtime.sh` automatically for a git-worktree workspace that has no explicit runtime provision command configured and is still `seed-pending`. An explicitly configured runtime provision command always takes precedence.
